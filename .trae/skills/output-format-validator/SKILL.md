...
| Full view 6 fields (R4) | Inventory Delta（字段 0）/ Inventory State / Party Condition / Relationship & Threat / Map Knowledge / Base Structure State |
| No ## titles (R5) | 不含 markdown 标题行 |
| Field order (R7) | Inventory Delta 在其余 5 正文字段之前 |

> v1.71 起完整视图为 6 字段（`Inventory Delta` 为字段 0；近五日主要事件已取消输出）；`validate_output.py` 内部以"Delta 标签 + 5 正文字段"实现。
