...
- **可消耗物资记录硬约束（v1.32 新增）**：食物、饮水、燃料、药品等可消耗物资**必须按 kg 记录**（如"大米×5kg"、"柴油×85kg"、"腌鱼×3kg"）。WSK 从 WM Scene 的"容器描述 + ≈kg"双轨渲染中提取净重值（剥离容器描述），仅记录 `{物资名}×{数值}kg`。不得记录容器类型（"一袋"、"半桶"）、不得使用非 kg 单位（"罐"、"瓶"、"份"）、不得估算 WM 未写出的重量值。详见 `0-2 §可消耗物资渲染规范 v1.32`。
