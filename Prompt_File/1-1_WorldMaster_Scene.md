...
- **移动路由地理准确性硬约束**（v1.25 新增）：跨子区域移动的 Route 必须严格按 `0-2 §分区邻接` 拓扑书写（工业↔中心↔东区↔东南等），不得凭印象虚构路径或跳过相邻区。涉及据点/势力/资源点时必须经过该点所在子区域。反例：起点 工业区/N，目标 码头调度楼（位于 东南区/C），Route 写 `工业区 N → 中心区 S 缘 → 东区 W → 东区 E 沿海仓储街` = 违规（跳过东南区，应包含 `东南区 N → 东南区 C`）；正确 Route：`工业区 N → 工业区 S → 东南区 N → 东南区 C（码头调度楼）→ 东南区 E → 东区 S → 东区 E（沿海仓储街）`。
