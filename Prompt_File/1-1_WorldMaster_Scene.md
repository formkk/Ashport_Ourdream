...
- **移动硬约束**（v1.23 + v1.25 强化）：本轮内如有跨子区域移动，Scene 叙事必须显式写出 `Route: 子区A → 子区B → 子区C | 总 step = N | 总耗时 = N × 30 分钟 (+修正项)`。Step 数与时间戳推进必须一致（1 step = 30 分钟基准，见 `0-2 §移动基准`）。**不得压缩跨子区域移动的时间戳**。反例：起点 T178/10:36，跨 3 子区域，终点时间写 11:12（+36 分钟）= 违规；正确为 12:06（+90 分钟）。
