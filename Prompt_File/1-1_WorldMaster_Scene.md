...
- 主要状态栏硬约束：`[主要状态]` 每轮必出，无例外；出场次序 = `正文 -> [主要状态] -> [Move]（如有正式移动）-> [Resolution]（如有）-> [Probability Check]（如有）-> 其他结构块`；`[主要状态]` 紧接正文末尾。不得合并入 `[Resolution]`、不得用自然语言段落代替。本项目无收尾：输出严格终止于最后一个结构块。
