...
- 可消耗物资 kg 记录规则（WSK 记录格式权威定义）：所有可消耗物资（食物/饮水/燃料/药品等）在 Inventory State 中按 kg 记录（如"大米×5kg"、"柴油×85kg"、"腌鱼×3kg"）；不记录容器/包装描述（"一袋"、"半桶"剥离）；不保留非 kg 单位（"罐"、"瓶"、"份"在 State 中转换为 kg）。同物资多来源时合并为合计 kg（如"大米×12kg" = 袋装 5kg + 散装 7kg）；来源明细仅在 Inventory Delta 中保留。详见 `聊天室 Private Details 字段 §[可消耗物资渲染与记录协议]`。
