...
```
[State Update]
D2-T5 / 13:15 / Afternoon / 工业区 N 化工厂保安室 / Clear Cold /party-known

Human Contact Status:
- 未知拾荒者(男性，约40岁): 初次接触进行中 | Trust=neutral | Hostility=low(目前克制) | 装备: 截短双管猎枪×1 + 猎刀×1 | 姿态: 要求分弹药，尚未举枪

Inventory Delta:
无新增变化(保安室搜刮已完成入账，遭遇对话中无物资交换)

Inventory Snapshot:
随身: 武器: 霰弹枪×1(泵动式，空膛)+转轮手枪×1(6发，空膛)+弹簧刀×1+螺丝刀×1+小刀×1; 弹药: 无(随身未携带备用弹药); 食品: 无; 医疗: 急救腰包×1(三角巾+止血带+弹性绷带×1+电解质冲剂×2); 燃料: 无; 工具: 手电筒×1+伸缩棍×1+防暴喷雾×1; 衣物: 警用冬夹克×1+旧皮靴×1; 价值物资: 香烟×2根; 杂物: 北区地图×1(手绘)+手铐×1+铜质哨子×1; 饮水: 搪瓷水壶×1(约800ml); 防盗门钥匙×2把
背包(随身容器): 弹药: 霰弹×38发(30发盒装+8发零散)+转轮手枪弹×12发+9mm×50发+.22×28发+.380×5发+7.62×39mm×10发; 其他: 枪油×1盒+擦枪布×1小包+磨刀石×1+旧工装夹克×1(备用)
据点(化工厂质检小楼): 物资不变(见上一轮完整账本)

Scavenging Status Snapshot:
工业区/N/化工厂保安室: 已搜刮1次 | 翻空程度: 低 | 上次搜刮: D2-T5

Survival Anchor Snapshot:
- 饮水: D2-T5 13:15(上午持续有饮水) | 进食: D2-T3 约07:30(煮大米饭一饭盒) | 睡眠: D2-T1 06:00(醒)
- 负重步行: 15 step(累计) | 劳动负荷: 轻-中 | 冷湿暴露: 无

Travel Time: 0(原地对峙) / Steps: 0

Recent Changes:
- Jiang在保安室内遭遇一名未知拾荒者(男性，约40岁)，对方手持截短双管猎枪，腰间挂猎刀
- 该拾荒者声称已盯保安室枪械柜两天，因锁锈死回去取撬棍，返回时发现Jiang已先进入
- 对方提出分配方案: "枪归你，子弹归我，公平分配"——要求获得霰弹，允许Jiang保留霰弹枪
- Jiang的霰弹枪和转轮手枪当前均为空膛，备用弹药全部在背包内
- 双方距离约3米，对方猎枪枪口垂向Jiang左膝方向，未直接瞄准
- 冲突尚未爆发，处于谈判阶段
```
...
[弹药口径格式硬约束]
- 弹药**按武器类别分组记录**（手枪弹 / 步枪弹 / 霰弹 / 其他口径），保留口径信息（如 `.22×28发 + 9mm×55发`）
- **禁止统一换算为"等价 9mm 单位"**合并记录（会导致武器填装信息丢失）
- **禁止用抽象数字代替口径**（如"手枪弹药组"代替"9mm×55发 + 转轮手枪弹×1发"）
