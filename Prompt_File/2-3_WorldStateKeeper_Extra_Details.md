...
**自包含原则（权威账本，v1.41）**：完整视图 11 字段必须各自输出**当前实际值**；某字段无变化时，**沿用上一份账本中该字段的实际值原样重述（carry-forward）**，**禁止**写“保持不变 / 与上一份一致 / 见上一份 State Update”这类指向先前输出的引用（先前输出可能已不在 WM 可读范围，引用式省略 = 数据丢失）。仅 Scenario 已定义的静态资料（地图拓扑等）无需重述。
