...
- 输出 = 全量快照
- **默认视图（v1.30 每次必出）**：`[State Update]` 软标签 + 第一行 + 变化子段（**仅 Inventory Delta / Recent Changes**，无变化时不写）+ **完整视图（10 字段全集，字段清单与顺序见下方 `[完整视图]` 段）**。Contamination 字段已删除（v1.30）。用户每次点击 WSK 都输出完整快照，确保 WM 每轮都能独立获取完整世界状态。
...
- **变化子段**（按需出现，无数据时不写）：**仅 `Inventory Delta:` / `Recent Changes:` 两个**。其余字段（即下方 `[完整视图]` 段所列 10 字段）**全部由完整视图统一承载，不得作为变化子段单独列出**。无变化时整段省略。
...
示例仅示第一行必备字段；变化子段仅 `Inventory Delta:` / `Recent Changes:` 两个（无变化时不写）；完整视图 10 字段全集见下方 `[完整视图]` 段。
...
**自包含原则（权威账本，v1.41）**：完整视图 10 字段必须各自输出**当前实际值**；某字段无变化时，**沿用上一份账本中该字段的实际值原样重述（carry-forward）**，**禁止**写“保持不变 / 与上一份一致 / 见上一份 State Update”这类指向先前输出的引用（先前输出可能已不在 WM 可读范围，引用式省略 = 数据丢失）。仅 Scenario 已定义的静态资料（地图拓扑等）无需重述。**唯一例外**：Base Structure State 日常无变化时输出简表（仅列组件名 + 当前 Condition）即为“当前实际值”，完整基线仅在首次建立/Delta/重排时输出。
...
6. Map Knowledge
7. Trade Obligation
8. Survival Anchor State
9. Base Structure State（含基础设施）
- 近五日主要事件（完整视图最末，按 D 升序）
- **Active Concerns（v1.33，v1.38 分类约束强化）**：位于变化子段之后、完整视图 10 字段之前：WSK 扫描全部字段，汇总当前最紧急的 3-5 项关注点。格式：`[类别] 问题描述`。
...
- **可消耗物资 kg 记录规则（v1.32，WSK 记录格式权威定义）**：所有可消耗物资（食物/饮水/燃料/药品等）在 Inventory State 中**按 kg 记录**（如"大米×5kg"、"柴油×85kg"、"腌鱼×3kg"）；**不记录容器/包装描述**（"一袋"、"半桶"剥离）；**不保留非 kg 单位**（"罐"、"瓶"、"份"在 State 中转换为 kg）。同物资多来源时合并为合计 kg（如"大米×12kg" = 袋装 5kg + 散装 7kg）；来源明细仅在 Inventory Delta 中保留。详见 `0-2 §可消耗物资渲染规范 v1.32`。
- **消耗投影（v1.33，v1.38 WM 驱动化）**：Inventory State 末尾，记录关键资源剩余天数估算，供 WM 快速判断生存紧迫度：
  ...
  - 基准值（仅 WM 未估算时兜底）：每人每天 ≈1kg 食物 / ≈2L(kg) 饮水；燃料为据点固定成本 ≈1-2kg/日（按取暖空间计，不随人数线性增长，冬季×2，详见 `0-2 §世界硬规则` 据点持续成本）
...
- 口径换算系数表（仅限交易场景使用，不改变 State 存储）：9mm/.45/.380 = 1 单位；.22/霰弹/7.62/5.45 = 0.5 单位；香烟 = 1 单位

模板声明:
1. 以上模板仅用于输出格式约束,不代表当前已成立事实。
2. 留空字段不是默认事实;WM Scene 叙事中无已成立变化时,不得依据模板自行补全字段值。
3. WM Scene 叙事中无已成立变化时,不生成正式状态,不补全模板字段。
4. 每次点击 WSK 必出完整视图（10 字段平铺列表，见上方 `[完整视图]` 段），**不使用 `##` / `###` 标题**；不区分“默认视图/完整视图/极简回执”模式——成功即输出完整快照（v1.30）。
...
7. 每次成功提交必须输出完整视图 10 字段（见上方 `[完整视图]` 段），不得省略。无变化可提取时不输出（静默）；信息不足时返回 `[Commit Rejected]`（v1.30）。
...
3. 完整视图 10 字段（平铺，不嵌套 `##` 标题）：按上方 `[完整视图]` 段编号顺序输出（近五日主要事件最末，按 D 升序）。
