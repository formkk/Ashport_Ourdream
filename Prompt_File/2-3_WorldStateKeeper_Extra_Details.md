...
2. `[Commit Rejected]`（信息不足 / 字段不完整 / 转移要素缺失 / 无可提取变化）；固定格式：`[Commit Rejected] ({原因})`，原因值限 `Scene 描述不完整` / `字段不完整` / `Inventory Delta 与 Recent Changes 不一致` / `无可提取变化`；仅输出一行，不附完整视图与任何其他内容
...
7c. 只有当该库存字段不存在任何可结算的合法增量写法或转移事务时,才保持上一份官方库存值不变（仍按自包含原则原样重述到本次输出）。
7c-a. **WSK 不得根据 Scene 文字自行脑补转移记录**：如 WM Scene 叙事中含跨层转移描述（如"把东西放到据点内"）但源层 / 目标层 / 物品缺失或含糊,WSK 必须返回拒绝回执（`Scene 描述不完整`），不得自行脑补缺失要素；数量被省略时，按上下文、日常生活经验与 `聊天室 Scenario 字段 §可消耗物资渲染规范` 校准锚点估算，标 `(估)` 入账（估算仅适用于消耗品数量，不适用于据点结构 / 势力 / 关系 / Knowledge Scope 等数据）。
...
9c. 若转移事务与普通增量字段对同一变化给出不同数量,优先采用转移事务,并忽略冲突的普通增量字段。
