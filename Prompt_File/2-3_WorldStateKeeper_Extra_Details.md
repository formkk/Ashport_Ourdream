...
- **Active Concerns（v1.33，位于变化子段之后、完整视图 11 字段之前）**：WSK 扫描全部字段，汇总当前最紧急的 3-5 项关注点。格式：`[类别] 问题描述`。类别使用 `[生存]`（饮水/食物/伤病/疲劳）/ `[人际]`（关系/敌对/交易/威胁）/ `[环境]`（天气/温度/路线/污染）/ `[据点]`（结构/维护/燃料）；无该项写"无"。仅列出正在活跃、需 WM 优先处理的项；稳定/已解决的问题不列入。
  - 示例：
    Active Concerns:
    - [生存] 饮水≈2天，需优先找水源
...
- **消耗投影（v1.33）**：Inventory Snapshot 末尾，WSK 根据队伍人数和已记录库存，估算关键资源剩余天数，供 WM 快速判断生存紧迫度：
  - 格式：`消耗估算（{N}人）：食物≈{X}天 / 饮水≈{Y}天 / 燃料≈{Z}天（冬季×2）`
  - 仅估算食物（主食+罐头+干粮合计 kg）、饮水（净水 kg）、燃料（柴油+煤炭+木柴合计，冬季消耗加倍）
  - 使用 `≈`（约等于），明确标注为估算值，不替代精确 kg 记录
  - 队伍人数从 Party Condition 获取；物资数据不足时写"数据不足"或省略
  - 估算基准（WM 可覆盖）：每人每天 ≈0.5kg 食物 / ≈2L(kg) 饮水 / 冬季取暖 ≈5kg 燃料（柴油等价）/ 非冬季 ≈1kg
