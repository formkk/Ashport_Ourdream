...
- 5 轨压力用自然语言 + 状态级（轨道名与状态级取值以 `0-2 §环境生存压力系统` 为权威；如 疲劳 strained / 脱水 critical）
- 不重复抑制机制的禁词与句式
...
- **Active Concerns（v1.33，v1.38 分类约束强化）**：位于变化子段之后、完整视图 11 字段之前：WSK 扫描全部字段，汇总当前最紧急的 3-5 项关注点。格式：`[类别] 问题描述`。
...
- **可消耗物资 kg 记录规则（v1.32，WSK 记录格式权威定义）**：所有可消耗物资（食物/饮水/燃料/药品等）在 Inventory Snapshot 中**按 kg 记录**（如"大米×5kg"、"柴油×85kg"、"腌鱼×3kg"）；**不记录容器/包装描述**（"一袋"、"半桶"剥离）；**不保留非 kg 单位**（"罐"、"瓶"、"份"在 Snapshot 中转换为 kg）。同物资多来源时合并为合计 kg（如"大米×12kg" = 袋装 5kg + 散装 7kg）；来源明细仅在 Inventory Delta 中保留。详见 `0-2 §可消耗物资渲染规范 v1.32`。
- **消耗投影（v1.33，v1.38 WM 驱动化）**：Inventory Snapshot 末尾，记录关键资源剩余天数估算，供 WM 快速判断生存紧迫度：
  ...
  - 基准值（仅 WM 未估算时兜底）：每人每天 ≈0.5kg 食物 / ≈2L(kg) 饮水 / 冬季取暖 ≈5kg 燃料（柴油等价）/ 非冬季 ≈1kg
