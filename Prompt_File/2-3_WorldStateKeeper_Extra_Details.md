...
**自包含原则（权威账本，v1.41）**：完整视图 9 字段必须各自输出**当前实际值**；仅 Scenario 已定义的静态资料（地图拓扑等）无需重述。
...
- **可消耗物资 kg 记录规则（WSK 记录格式权威定义）**：所有可消耗物资（食物/饮水/燃料/药品等）在 Inventory State 中**按 kg 记录**（如"大米×5kg"、"柴油×85kg"、"腌鱼×3kg"）；**不记录容器/包装描述**（"一袋"、"半桶"剥离）；**不保留非 kg 单位**（"罐"、"瓶"、"份"在 State 中转换为 kg）。同物资多来源时合并为合计 kg（如"大米×12kg" = 袋装 5kg + 散装 7kg）；来源明细仅在 Inventory Delta 中保留。详见 `聊天室 Private Details 字段 §可消耗物资渲染与记录协议`。
...
7d. **Inventory Delta 与 Recent Changes 一致性硬约束**：Inventory Delta 是本轮**所有库存变化**的纯记账汇总；Recent Changes 中描述的每一个消耗 / 获得 / 转移 / 丢失 / 赠送 / 交易事件，必须在 Inventory Delta 中可追溯；如两者不一致（Delta 写“无新增”但 Recent Changes 明确消耗 / 转移），WSK 必须返回拒绝回执（`Inventory Delta 与 Recent Changes 不一致`），不得提交成不完整的账本。
