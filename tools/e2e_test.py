...
[移动] 工业区/N -> 工业区/C -> 工业区/E | Steps: 2 | Travel Time: 60min
[掷骰] Trigger: third-party-sighting: 穿越工业区开阔地带是否被第三方远距目击；Base: 15（白天开阔地人流低）；Modifiers: 开阔地+10，白天人流低-5；Final: 20；Seed: 42；Threshold: 20；Result: 未触发；Outcome: 疑似跟踪者放弃，未升级接触
...
def check_output(label, output):
    ...
    for i, line in enumerate(lines):
        for block_name in ['[移动]', '[掷骰]', '[判定]', '[主要状态]']:
    ...
    # 4. 检查空块格式
    move_line = lines[blocks_found.get('[移动]', -1)] if '[移动]' in blocks_found else ""
    if '[移动] 无' in move_line:
        checks.append("  ✅ [移动] 空格式正确: '[移动] 无'")

    prob_line = lines[blocks_found.get('[掷骰]', -1)] if '[掷骰]' in blocks_found else ""
    if '无概率事件' in prob_line:
        checks.append("  ✅ [掷骰] 空格式正确: '无概率事件'")
    ...
    if '；' in prob_line and 'Trigger' in prob_line:
        checks.append("  ✅ [掷骰] 各项用 ；分隔")

    # 8. json SSOT 格式校验：用 output_rules.json 的 format_pattern 校验全部结构块行
    for block_name, line in [('[移动]', move_line), ('[掷骰]', prob_line),
...
def check_regex_negative_cases():
    ...
    pc_pattern = RULES['wm']['block_rules']['[掷骰]']['format_pattern']

    # 4 个真实 WM 输出样本（用户提供，2026-08 采集；末尾 * 已去除以隔离单一变量）
    real_samples = [
        "[掷骰] Trigger: 江在护林员小屋内搜刮，屋外左侧再次出现明显擦碰声，附近是否有人正接近或已发现小屋被入；Base: 30（雨林地带声近且透）；Modifiers: 江入屋后动作收敛-5，小屋半塌有回风干扰+5，异响极近且无风+15；Final: 45；Seed: 33；Threshold: 45；Result: 触发；Outcome: 屋外有声源，但未冲门，也未现形。江判断不可再搜，风险转为即时不安；未与之照面，仅一瞬后复静。",
        "[掷骰] Trigger: 从游客中心前往护林员小屋的短暂林路，是否被林内其他人先看到或跟上；Base: 15（林中遮掩多，江一路压步子）；Modifiers: 鸟群惊飞后江警觉+5，林间腐叶厚难避免细微响动-5，天阴能见度差但声音反传清晰-0；Final: 15；Seed: 58；Threshold: 15；Result: 未触发；Outcome: 江抵达小屋外，暂无人现身，但林路上确有来源不明的细响，需谨慎进屋。",
        "[掷骰] Trigger: 进入游客中心搜索并传出轻微翻找声，同时鸟群惊飞，是否有人或兽已察觉江并朝建筑靠近；Base: 25（游客中心为显眼地标，附近已现异动）；Modifiers: 江动作轻-5，树林遮蔽与天色阴-5，鸟群惊飞说明近处已有扰动+15；Final: 30；Seed: 48；Threshold: 30；Result: 未触发；Outcome: 未发现人影与生物逼近，但鸟群惊飞方向正对所搜房间东侧，现场紧张感陡增。",
        "[掷骰] Trigger: 江从北区住宅带前往森林公园边缘，经过林缘锈栏时，林内是否有人先发现他；Base: 20（林缘开阔段+北区近郊偶有流民）；Modifiers: 江单人轻装-5，昼间林鸟未惊-5，天阴多影+5；Final: 15；Seed: 76；Threshold: 15；Result: 未触发；Outcome: 未发现林中人迹，仅一声枯枝响，来源不明；但森林公园整体气压低，落叶腐味像有活物在深处。",
    ]
    ...
    valid_cases = [
        ...
        "[掷骰] Dase-1；Dase-4",
        "[掷骰] wear-weather: 未触发",
        "[掷骰] wear-weather: 未触发；wear-sabotage: 未触发",
        "[掷骰] Trigger: hostile-contact: 江在护林员小屋内搜刮，屋外左侧再次出现明显擦碰声，是否有人接近或已发现小屋被入；Base: 30（雨林地带声近且透）；Modifiers: 江入屋后动作收敛-5，小屋半塌有回风干扰+5，异响极近且无风+15；Final: 45；Seed: 33；Threshold: 45；Result: 触发；Outcome: 屋外确认有声源，未冲门，未现形，一瞬后复静，江未与之照面",
        "[掷骰] Trigger: hostile-contact#2: 同轮第二次独立敌对接触检查；Base: 20；Modifiers: 夜间+10；Final: 30；Seed: 55；Threshold: 30；Result: 未触发；Outcome: 无接触",
        "[掷骰] Trigger: custom: 自定义事件检查；Base: 10；Modifiers: 无关紧要+5；Final: 15；Seed: 22；Threshold: 15；Result: 未触发；Outcome: 无事发生",
    ]
