...
| 角色生成时可见 | 不可见 |
|---|---|
| 聊天室级字段（Private Details + Scenario）+ 本角色卡三字段（Scene / Additional Personality Details / Extra Details）+ 对话历史（含对方实际输出） | 对方角色卡内容（World Master 看不到 World State Keeper 的字段，反之亦然） |

**关键推论**：
1. 角色间共享信息的唯一运行时通道是**对话历史中的实际输出**（如 WSK 读 World Master 的 [主要状态] 输出、World Master 读最新 [State Update]），不是对方角色卡定义。
2. 开发侧文件编号（0-1/0-2/1-x/2-x）与开发简称（WM/WSK）**在平台里不存在**，不得作为引用目标或出现在引用字符串中。

**引用写法规范**（全部提示词文件已按此统一，2026-07-24）：
...
**审计方法**：grep 所有 `§` 引用与 `0-[12]/1-[123]/2-[123]` 模式，逐条判断"引用方角色生成时能否看到被引用内容"；不可见的必须内联规则或改指实际输出。
